...
data = {
    "headers":
    [{"name": "dx", "column": "Data"},{"name": "pe", "column": "Period"},{"name": "ou", "column": "Organisation unit"},{"name": "value", "column": "Value"}], "metaData": {"items": {"202108": {"name": "August 2021"},
...
def getNestedRow(dx , items ):
    ...
    for item in items:
        if item in dx:
            # print(item)
            nested_rows.append(items[item]['name'])      
    return nested_rows
